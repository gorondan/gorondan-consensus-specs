# Phase 0 --> Altair --> Bellatrix -- The Beacon Chain
...
- [Phase 0 --\> Altair --\> Bellatrix -- The Beacon Chain](#phase-0----altair----bellatrix----the-beacon-chain)
  ...
  - [Initialize state for pure Bellatrix testnets and test vectors](#initialize-state-for-pure-bellatrix-testnets-and-test-vectors)
...
The spec does *not* override previous configuration values but instead creates new values and replaces usage throughout.
...
Refer to the extensions defined in the [Altair BLS document](./bls.md). This specification assumes knowledge of the functionality described in those documents.
...
*Bellatrix Note*: The function `process_slashings` is modified to use `PROPORTIONAL_SLASHING_MULTIPLIER_BELLATRIX`.
...
*Bellatrix Note*: The call to the `process_execution_payload` must happen before the call to the `process_randao` as the former depends on the `randao_mix` computed with the reveal of the previous block.
...
This helper function is only for initializing the state for pure Bellatrix testnets and tests.

*Altair Note*: The function `initialize_beacon_state_from_eth1` is modified: (1) using `ALTAIR_FORK_VERSION` as the previous and current fork version, (2) utilizing the Altair `BeaconBlockBody` when constructing the initial `latest_block_header`, and (3) adding initial sync committees.

*Bellatrix Note*: The function `initialize_beacon_state_from_eth1` is modified for pure Bellatrix testing only.
